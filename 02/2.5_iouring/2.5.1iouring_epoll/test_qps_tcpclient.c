...
int main(int argc,  char *argv[]) {
    ...
    while ((opt = getopt(argc, argv, "s:p:t:c:n:")) != -1) {
        switch (opt)
        {
            ...
            case 'c': // 连接tcpserver的次数
                printf("-c: %s\n", optarg);
                ctx.connection = atoi(optarg);
                break;

            case 'n':// send_recv_tcppkt的次数
                printf("-n: %s\n", optarg);
    ...
    // pthread ptid[10] = {0}; 这种方式是定义在栈上面，而下面是在堆上面，处理的方式是一样的。
    pthread_t *ptid = malloc(ctx.threadnum * sizeof(pthread_t)); // 动态创建线程id数量，需要在结束时释放内存
    int i =0;
    ...
    printf("success: %d, failL: %d, time_used: %d, qps: %d\n", 
